...
if ( is_admin() ) {
	...
	function woo_ce_extend_subscription_dataset_args( $args, $export_type = '' ) {
		...
		if ( $user_count < $list_limit ) {
			$args['subscription_customer'] = ( isset( $_POST['subscription_filter_customer'] ) ? array_map( 'absint', (array) $_POST['subscription_filter_customer'] ) : false );
		} else {
			...
			foreach ( $gf_fields as $gf_field ) {
				...
				if ( $gf_field_duplicate !== true ) {
					$fields[] = array(
						...
						'label' => sprintf( apply_filters( 'woo_ce_extend_order_fields_gf_label', __( 'Subscription Items: %1$s - %2$s', 'woocommerce-exporter' ) ), ucwords( strtolower( $gf_field['formTitle'] ) ), ucfirst( strtolower( $gf_field['label'] ) ) ),
