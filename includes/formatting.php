...
function woo_ce_detect_value_string( $string = false, $format = 'boolean' ) {
	...
	if ( $export->escape_formatting == 'all' ) {
		if ( WOO_CE_LOGGING && apply_filters( 'woo_ce_debug_detect_value_string', false ) ) {
...
function woo_ce_format_custom_meta( $custom_meta = false ) {
	...
	$output = $custom_meta;
	if ( WOO_CE_LOGGING && apply_filters( 'woo_ce_debug_format_custom_meta', false ) ) {
	...
	if ( ! empty( $custom_meta ) ) {
		...
		if ( is_array( $custom_meta ) ) {
			...
			for ( $i = 0; $i < $size; $i++ ) {
				...
				} elseif ( is_a( $custom_meta[ $i ], 'WC_Meta_Data' ) ) {
					$output .= $custom_meta[ $i ]->get_data()['value'] . $export->category_separator;
				} else {
...
function woo_ce_filter_wc_price( $return, $price ) {
	...
	if ( strstr( $return, '<span class="woocommerce-Price-amount amount"><bdi>-' ) || strstr( $return, '<span class="woocommerce-Price-amount amount">-' ) ) {
		$price = '-' . $price;
    }
			...
			if ( isset( $woocommerce->countries ) ) {
				if ( $states = $woocommerce->countries->get_states( $country_prefix ) ) {
					if ( isset( $states[ $state_prefix ] ) ) {
						$output = html_entity_decode( $states[ $state_prefix ] );
                    }
				}
				unset( $states );
			}
...
function woo_ce_format_description_excerpt( $string = '' ) {
	...
	$description_excerpt_formatting = get_option( WOO_CE_PREFIX . 'woo_ce_description_excerpt_formatting', 0 );
	if ( $description_excerpt_formatting ) {
