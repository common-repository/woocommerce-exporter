...
if ( is_admin() ) {
	...
	if ( ! function_exists( 'woo_ce_get_export_type_review_count' ) ) {
		function woo_ce_get_export_type_review_count() {
			...
			$cached = get_transient( WOO_CE_PREFIX . '_review_count' );
			if ( $cached == false ) {
				$args    = array(
					'count'       => true,
					'status'      => 'all',
				...
				$reviews = get_comments( $args );
				$count   = absint( $reviews );
				set_transient( WOO_CE_PREFIX . '_review_count', $count, HOUR_IN_SECONDS );
			} else {
				$count = $cached;
			}
			return $count;
	...
	function woo_ce_review_dataset_args( $args, $export_type = '', $form_data = array() ) {
		if ( empty( $form_data ) ) {
			$form_data = $_POST;
		}
		...
		$defaults = array(
			'review_orderby' => ( isset( $form_data['review_orderby'] ) ? sanitize_text_field( $form_data['review_orderby'] ) : false ),
			'review_order'   => ( isset( $form_data['review_order'] ) ? sanitize_text_field( $form_data['review_order'] ) : false ),
		);
...
function woo_ce_cron_review_dataset_args( $args, $post_id, $export_type = '', $is_scheduled = 0 ) {
...
add_filter( 'woo_ce_extend_cron_dataset_args', 'woo_ce_cron_review_dataset_args', 10, 4 );
...
function woo_ce_get_reviews( $args = array(), $export = array() ) {
	...
	if ( 'last_export' === $review_dates_filter ) {
