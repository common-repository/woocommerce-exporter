...
function woo_ce_export_template_banner( $post ) {
    ...
    if ( apply_filters( 'woo_ce_export_template_banner_save_prompt', true ) ) {
        ...
        ) . '" id="return-button" class="button confirm-button" data-confirm="' . esc_html__( 'The changes you made will be lost if you navigate away from this page before saving.', 'woocommerce-exporter' ) . '" data-validate="yes">' . esc_html__( 'Return to Export Templates', 'woocommerce-exporter' ) . '</a>';
    } else {
echo '<a href="' . esc_url(
    add_query_arg(
        array(
...
) . '" id="return-button" class="button">' . esc_html__( 'Return to Export Templates', 'woocommerce-exporter' ) . '</a>';
    }
    ...
    if ( ! woo_ce_get_option( 'dismiss_max_input_vars_prompt', 0 ) ) {
        ...
        $message     = '<span style="float:right;"><a href="' . $dismiss_url . '" class="woocommerce-message-close notice-dismiss">' . esc_html__( 'Dismiss', 'woocommerce-exporter' ) . '</a></span>';
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                ...
                var max_fields = '<?php echo esc_html( $total ); ?>';
    ...
    <div id="export_template_options" class="panel-wrap export_template_data">
        ...
        <ul class="coupon_data_tabs wc-tabs" style="display:none;">
            ...
            $coupon_data_tabs = apply_filters(
                'woo_ce_export_template_data_tabs',
                array(
					'general' => array(
            ...
            ?>
                <li class="<?php echo esc_attr( $key ); ?>_options <?php echo esc_attr( $key ); ?>_tab <?php echo esc_attr( implode( ' ', (array) $tab['class'] ) ); ?>">
                    <a href="#<?php echo esc_url( $tab['target'] ); ?>"><?php echo esc_html( $tab['label'] ); ?></a>
                </li>
                <?php
                    }
                    ?>
...
function woo_ce_export_template_general_export_type( $post_ID = 0 ) {
    ...
    <div class="options_group">
        <p class="form-field"><?php esc_html_e( 'Select an Export type then switch to the Fields tab to select your export field preferences. You can save export field preferences for multiple Export Types. Click Publish or Update to save changes.', 'woocommerce-exporter' ); ?></p>
        <p class="form-field discount_type_field ">
            ...
            <?php if ( ! empty( $export_types ) ) { ?>
                ...
                <img class="help_tip" data-tip="<?php esc_html_e( 'Select the export type fields you want to manage.', 'woocommerce-exporter' ); ?>" src="<?php echo esc_url( WC()->plugin_url() ); ?>/assets/images/help.png" height="16" width="16" />
            <?php } else { ?>
...
function woo_ce_export_template_save( $post_ID = 0 ) {
    ...
    if ( ! empty( $export_types ) ) {
        ...
        foreach ( $export_types as $export_type ) {
            $fields  = ( isset( $_POST[ sprintf( '%s_fields', $export_type ) ] ) ? array_map( 'sanitize_text_field', (array) $_POST[ sprintf( '%s_fields', $export_type ) ] ) : false );
            $sorting = ( isset( $_POST[ sprintf( '%s_fields_order', $export_type ) ] ) ? array_map( 'absint', (array) $_POST[ sprintf( '%s_fields_order', $export_type ) ] ) : false );
            $labels  = ( isset( $_POST[ sprintf( '%s_fields_label', $export_type ) ] ) ? array_map( 'sanitize_text_field', (array) $_POST[ sprintf( '%s_fields_label', $export_type ) ] ) : false );
            if ( ! empty( $labels ) ) {
