...
if( is_admin() ) {
	...
	function woo_ce_coupon_scheduled_export_save( $post_ID = 0 ) {
		...
		update_post_meta( $post_ID, '_filter_coupon_discount_type', ( isset( $_POST['coupon_filter_discount_type'] ) ? array_map( 'sanitize_text_field', (array)$_POST['coupon_filter_discount_type'] ) : false ) );

	}
	add_action( 'woo_ce_extend_scheduled_export_save', 'woo_ce_coupon_scheduled_export_save' );

	function woo_ce_coupon_dataset_args( $args, $export_type = '', $form_data = array() ) {
		...
		$defaults = array(
			'coupon_discount_type' => ( isset( $form_data['coupon_filter_discount_type'] ) ? woo_ce_format_product_filters( array_map( 'sanitize_text_field', (array)$form_data['coupon_filter_discount_type'] ) ) : false ),
			'coupon_orderby' => ( isset( $form_data['coupon_orderby'] ) ? sanitize_text_field( $form_data['coupon_orderby'] ) : false ),
			'coupon_order' => ( isset( $form_data['coupon_order'] ) ? sanitize_text_field( $form_data['coupon_order'] ) : false ),
		);
...
function woo_ce_cron_coupon_dataset_args( $args, $post_id, $export_type = '', $is_scheduled = 0 ) {
...
add_filter( 'woo_ce_extend_cron_dataset_args', 'woo_ce_cron_coupon_dataset_args', 10, 4 );
...
function woo_ce_get_coupons( $args = array(), $export = array() ) {
	if ( empty( $export ) ) {
		global $export;
	}
...
function woo_ce_get_coupon_data( $coupon_id = 0, $args = array(), $export = array() ) {
