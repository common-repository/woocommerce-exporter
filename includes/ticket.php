...
if( is_admin() ) {
	...
	if( !function_exists( 'woo_ce_get_export_type_ticket_count' ) ) {
		function woo_ce_get_export_type_ticket_count() {
			...
			$cached = get_transient( WOO_CE_PREFIX . '_ticket_count' );
			if( $cached == false ) {
				$args = array(
					'post_type' => $post_type,
					'posts_per_page' => 1,
				...
				$count_query = new WP_Query( $args );
				$count = $count_query->found_posts;
				set_transient( WOO_CE_PREFIX . '_ticket_count', $count, HOUR_IN_SECONDS );
			} else {
				$count = $cached;
			}
			return $count;
...
function woo_ce_get_tickets( $args = array(), $export_settings = null ) {
