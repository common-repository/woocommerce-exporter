...
function woo_ce_get_product_vendors( $args = array(), $output = 'term_id', $export = null ) {
...
function woo_ce_get_product_vendor_data( $vendor_id = 0, $args = array(), $export_settings = null ) {
	global $export;
	if ( empty( $export ) ) {
...
function woo_ce_get_product_assoc_product_vendor_commission( $product_id = 0, $vendor_ids = array() ) {
	...
	if( !empty( $product_commission ) ) {
		...
		if( !empty( $vendor_ids ) ) {
			...
			$size = is_array($vendor_ids) ? count( $vendor_ids ) : 1;
			if( $size == 1 )
				$vendor_ids = array( $vendor_ids );
			for( $i = 0; $i < $size; $i++ ) {
				// Use get_commission_parent() as default and use P 	ost meta as fall-back
...
function woo_ce_format_product_vendor_users( $users = null, $return = 'user_login' ) {
	...
	if( !function_exists( 'get_vendor' ) ) {
		$users = !is_array($users) ? explode( ',', $users ) : $users;
		if( !empty( $users ) ) {
